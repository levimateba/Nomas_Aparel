        ...
        <form method="POST" action="{{ route('admin.price.store') }}">
            ...
            <select name="plan_template" id="plan_template" required>
                @foreach($pricingTemplates as $key => $template)
                    <option value="{{ $key }}" {{ old('plan_template', 'basic') === $key ? 'selected' : '' }}>{{ $template['label'] }}</option>
                @endforeach
            </select>

            <div id="custom_title_wrap" style="display:{{ old('plan_template') === 'custom' ? 'block' : 'none' }};">
                <label>Custom Plan Title</label>
                <input type="text" name="custom_title" value="{{ old('custom_title') }}" placeholder="e.g. NGO Systems Package">
            </div>

            <label>Plan Subtitle</label>
            <input type="text" name="subtitle" id="price_subtitle" value="{{ old('subtitle') }}" placeholder="e.g. Ideal for startups & small businesses">

            <label>Description</label>
            <textarea name="description" id="price_description" rows="3">{{ old('description') }}</textarea>
            <label>Amount (Kshs.)</label>
            <input type="number" step="0.01" name="amount" value="{{ old('amount') }}" required>
            <label>Display Price (shown on card)</label>
            <input type="text" name="display_price" id="display_price" value="{{ old('display_price') }}" placeholder="e.g. KES 15,000 - 50,000">
            <label>Billing Period</label>
            <input type="text" name="billing_period" value="{{ old('billing_period','project') }}" required>
            
            <label style="margin-top: 20px;">Features (one per line)</label>
            <textarea name="features_text" id="features_text" rows="6" placeholder="Basic consultation&#10;Website / small system setup&#10;Email support&#10;1-2 revisions">{{ old('features_text') }}</textarea>
