...
<div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    <div>
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">Edit Contact</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Keep public contact information accurate and easy to understand.</p>
    </div>
...
@section('content')
<div class="ta-page">
    <form method="POST" action="{{ route('admin.contact.update', $contact) }}" class="grid gap-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(240px,0.8fr)]">
        @csrf
        @method('PUT')
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="mt-5 space-y-4">
                <div class="ta-field">
                    ...
                    <input id="contact-label" class="ta-input" type="text" name="label" value="{{ old('label', $contact->label) }}" maxlength="255" placeholder="e.g. Customer Support" required>
                ...
                <div class="ta-field">
                    ...
                    <select id="contact-type" class="ta-select" name="type" required>
                        <option value="text" @selected(old('type', $contact->type) === 'text')>Text</option>
                        <option value="email" @selected(old('type', $contact->type) === 'email')>Email</option>
                        <option value="phone" @selected(old('type', $contact->type) === 'phone')>Phone</option>
                        <option value="address" @selected(old('type', $contact->type) === 'address')>Address</option>
                    </select>
                </div>
                <div class="ta-field">
                    <label for="contact-value">Value</label>
                    <input id="contact-value" class="ta-input" type="text" name="value" value="{{ old('value', $contact->value) }}" maxlength="255" placeholder="e.g. info@example.com" required>
                    <p class="mt-1 text-xs text-gray-500" id="contact-value-help">Provide the actual contact value users should use.</p>
                </div>
            </div>
            <div class="mt-6 flex flex-wrap gap-2">
                <button class="ta-btn" type="submit">Update contact</button>
