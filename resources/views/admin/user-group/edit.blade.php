...
<div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
    <div>
        ...
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">Edit User Group</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Update {{ $group->name }}.</p>
    </div>
...
<div class="ta-page" style="max-width:720px;">
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <form method="POST" action="{{ route('admin.user-groups.update', $group) }}" class="space-y-5 p-5">
            @csrf
            @method('PUT')
            <div class="ta-field">
                <label for="name">Name *</label>
                <input id="name" type="text" name="name" class="ta-input" required maxlength="255" value="{{ old('name', $group->name) }}">
            </div>
            <div class="ta-field">
                <label for="description">Description</label>
                <textarea id="description" name="description" rows="3" class="ta-input" maxlength="1000">{{ old('description', $group->description) }}</textarea>
            </div>
            <div class="flex flex-wrap gap-2">
                <button type="submit" class="ta-btn">Update group</button>
